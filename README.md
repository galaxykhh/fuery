...
- **One idea to learn.** A query is a definition. You pass the same object to a widget, fetch it with the client, and read its cached data. A mutation is a definition too, and any widget runs it with `addTodo.mutate('Buy milk', context.queryClient)`.
