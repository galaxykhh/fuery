...
- **Refetches on its own.** Fuery refetches stale data when a screen starts using it and when the app returns to the foreground. With a connectivity source, it also refetches when the network reconnects ([When the network reconnects](../guides/lifecycle/#when-the-network-reconnects)).
