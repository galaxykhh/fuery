...
The Queries tab lists every query with its key, its status, and how many widgets or streams use it. Type in the filter to find a key.

| Status | Meaning |
|---|---|
| `fetching` | A fetch is running |
| `paused` | A fetch is waiting for the network |
