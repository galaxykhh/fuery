...
With hundreds of queries, build the list with `useMemoized`, keyed on the ids. A rebuild with the same ids then passes the same list, and `useQueries` skips updating the queries:
