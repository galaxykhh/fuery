...
A `QueryClient` owns the query cache and the mutation cache, and every read, write, and refetch of cached data goes through it. The query cache keeps one cache entry (`CachedQuery`) for each query key: the data and state of that key. The mutation cache keeps one run (`CachedMutation`) for each `mutate` call. For tasks, see [Reading and updating the cache](../../guides/query-client/) and [Setting up the client](../../guides/client-setup/).

## Constructor options

| Name | Type | Default | Description |
|---|---|---|---|
| `queryCache` | `QueryCache` | `QueryCache()` | Holds the cache entries. Pass one to give it a [`QueryCacheConfig`](#cache-callbacks). |
...
| `onUncaughtError` | `void Function(Object error, StackTrace stackTrace)?` | `null` | Receives the errors that no caller can catch. Without it, they go to the current zone. See [Catching errors that callbacks throw](../../guides/client-setup/#catching-errors-that-callbacks-throw). |
...
| `setQueryData<TData>(queryKey, data, {updatedAt})` | `TData` | Writes the data by key. A cache entry this creates has no query function, so refetches skip it until an observer or `client.query` brings a query for the key. |
...
| `watch<T>(selector)` | `Stream<T>` | A broadcast stream of `selector(client)`. |

`watch` gives each listener the current value first. It emits again after the query cache or the mutation cache changes, when the new value differs. It compares lists, maps, and sets by content and other values with `==`. An error the selector throws goes to the stream. Watching fetches nothing.
...
| `infiniteQuery(query)` | `Future<InfiniteData<TPage, TParam>>` | `query` for an `InfiniteQuery`. With nothing cached, the fetch loads the query's `pages` (default 1, at most `maxPages`). With pages cached, it reloads them from the first, up to `maxPages`. |
...
- Fuery retries a failed fetch only when the query, `defaultOptions`, or a `setQueryDefaults` call for its key sets `retry`.
...
`invalidateQueries`, `refetchQueries`, and `resetQueries` don't refetch a cache entry that is:

- Disabled: its `isDisabled` is `true`.
- Static and has data: an observer uses `staleTime: staticStaleTime`.
- Written only by `setQueryData`, so it has no query function yet.
...
|---|---|---|---|
| `queryKey` | `List<Object?>?` | every cache entry | Cache entries whose key starts with this key. `['todos']` matches `['todos', 1]`. |
| `exact` | `bool` | `false` | With `true`, only the cache entry whose key equals `queryKey`. |
| `type` | `QueryTypeFilter` | `QueryTypeFilter.all` | `.active`: at least one enabled observer uses the cache entry. `.inactive`: none does. |
| `stale` | `bool?` | `null` | `true` selects stale cache entries, `false` fresh ones. |
| `predicate` | `bool Function(CachedQuery<Object> query)?` | `null` | Cache entries this function returns `true` for. |

`invalidateQueries` takes `type` as a `QueryTypeFilter?`, default `null`. Unset, it marks every match stale, as `.all` does, but refetches only the active matches. With `.all`, it refetches the inactive matches too.

`QueryFilters`, which `queryCache.find` and `findAll` take, has these fields plus `fetchStatus`, a `FetchStatus?` that selects cache entries in that fetch status. `matches(query)` tests one cache entry.
...
| `cancelRefetch` | `bool` | `true` | Cancels the fetch in flight of a cache entry that has data, and starts a new one. With `false`, waits for the fetch in flight. A cache entry loading its first data always keeps its fetch. |
| `throwOnError` | `bool` | `false` | With `true`, the returned future fails when a refetch fails. |
| `revert` | `bool` | `true` | Puts a cancelled cache entry back in the state it had before the fetch. With `false`, records the `CancelledError` as its error. |
| `silent` | `bool` | `false` | With `true` and `revert: false`, records no error: the cache entry keeps its state and returns to idle. |
...
A cache takes its config in its constructor and keeps it for its whole life, as `config`.

`QueryCacheConfig` runs its callbacks for every fetch in the query cache. Each callback returns `void` and receives the cache entry (`CachedQuery<Object>`) last:
...
- A run that `restore` restarts skips both `onMutate` callbacks.
...
| `queryCache.findAll([filters])` | `List<CachedQuery<Object>>` | Every match, or every cache entry without filters. |
| `queryCache.get(queryHash)` | `CachedQuery<Object>?` | The cache entry with this `queryHash`. |
...
| `queryHash` | `String` | The hash of the key, which identifies the cache entry in its cache. |
| `state` | `QueryState<TData>` | See [QueryState fields](#querystate-fields). |
| `options` | `Query<TData>` | The options the cache entry fetches with, defaults applied. |
| `meta` | `Map<String, Object?>?` | `options.meta`. |
| `observers` | `List<QueryObserver<TData>>` | The observers using the cache entry, in the order they subscribed. |
| `observersCount` | `int` | How many observers use the cache entry. |
| `isActive` | `bool` | At least one observer is enabled. |
| `isDisabled` | `bool` | The cache entry won't fetch on its own: every observer is disabled, or nothing observes it and `isFetched` is `false`. |
| `isStale` | `bool` | Stale for at least one observer. Without observers, `true` when it has no data or is invalidated. |
| `isStatic` | `bool` | An observer uses `staticStaleTime`, so the cache entry is never stale. |
| `isFetched` | `bool` | The cache entry received data or an error at least once, from a fetch or a write such as `setData`. Data restored from storage doesn't count. |
...
`AnyCachedMutation` is `CachedMutation<Object?, Object?, Object?>`, a run whose types the caller doesn't know. Filters and cache callbacks receive runs as this type. `AnyMutation` is `Mutation<Object?, Object?, Object?>`, the type of each definition that `restore(mutations:)` takes.

## QueryState fields

`QueryState<TData>` is the state a cache entry holds. Observers build each `QueryResult` from it.

| Field | Type | Description |
|---|---|---|
| `data` | `TData?` | The last data the cache entry received. `null` means no data. |
...
| `dataUpdateCount` | `int` | How many times the cache entry received data, from a fetch or a write such as `setData`. Data restored from storage doesn't count. |
...
| `restore({mutations})` | `Future<void>` | Reads every persisted query ahead of time, and restarts the stored runs of the mutations you pass. Does nothing without a storage. See [Restoring ahead of time](../../guides/persistence/#restoring-ahead-of-time). |
| `clear()` | `void` | Removes every cache entry and mutation run, and deletes all persisted data. Keeps the client and its per-key defaults. See [Clearing everything at logout](../../guides/query-client/#clearing-everything-at-logout). |
