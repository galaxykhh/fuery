...
- To release, bump every version and add a short CHANGELOG entry to each, merge to `main`, then push a tag `vX.Y.Z`. `.github/workflows/publish.yml` tests and publishes the packages, core first, and creates a GitHub release from their `## X.Y.Z` CHANGELOG entries. Don't publish or create releases by hand.
