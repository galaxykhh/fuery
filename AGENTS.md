...
- `packages/fuery_hooks`: `useQuery`, `useInfiniteQuery`, `useMutation`, and `useQueries` for `flutter_hooks`. Re-exports `fuery`.
...
- To release, bump every version and add a short CHANGELOG entry to each, merge to `main`, then push a tag `vX.Y.Z`. `.github/workflows/publish.yml` tests and publishes the packages, core first, and creates a GitHub release from their `## X.Y.Z` CHANGELOG entries. Don't publish or create releases by hand, except the first version of a new package, which pub.dev can't publish automatically.
