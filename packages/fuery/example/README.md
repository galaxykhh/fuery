...
| [Search](lib/app/screens/search/search_screen.dart) | Search as you type | The term lives in `setState`, and the widgets get `searchQuery(term)` on every build, keeping their observers. `keepPreviousData` keeps the last results on screen while the next ones load, and an empty term is `enabled: false`. |
