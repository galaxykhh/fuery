...
| [Compose](lib/app/screens/compose/compose_screen.dart) | Write a post | `Mutation.observe` with the feed invalidation in the mutation and the screen's own `onSuccess` at the call site. The new post polls with `refetchInterval` until `refetchWhile` sees it published, then stops. |
