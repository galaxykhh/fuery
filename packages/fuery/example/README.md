...
- [`data/feed_mutations.dart`](lib/app/data/feed_mutations.dart): the same for mutations. The cache work is written once, with the client each callback receives. Each widget that runs a mutation gets its own pending and error state, and a `mutationKey` lets any screen show every run.
