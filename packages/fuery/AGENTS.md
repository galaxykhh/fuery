...
- In debug builds, `didUpdateWidget` warns once per key (`debugWarnRecreated`, a `debugPrint`, never an error) when a widget gets a different observer with the same key: that is `Query.observe` in `build`. A different key, or the same instance, is silent.
