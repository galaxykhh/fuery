...
- Every widget (`Query*`, `InfiniteQuery*`, `Mutation*` × `Builder` / `Listener` / `Consumer`) is a thin `StatelessWidget` over `ResultSubscriber` in `lib/src/result_subscriber.dart`. New widget kinds should reuse it.
