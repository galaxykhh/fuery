...
- `lib/src/devtools.dart` holds `FueryDevtools` (button and panel over the app, off when `kReleaseMode`) and `FueryDevtoolsPanel`. The panel brings its own `Localizations`, `Theme`, and `Overlay`, so it works in an app's `builder` above the navigator. It only reads the caches and calls public `QueryClient` methods.
