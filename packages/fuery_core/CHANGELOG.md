...
- Fix: two observers that an observer subclass makes `==` both hear the query's changes, as a query now keeps its observers by identity. Before, the second heard none of them, and unsubscribing it detached the first.
