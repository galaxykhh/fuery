...
- Fix: a query keeps every observer that subscribes to it, also two that an observer subclass makes `==`. Before, the second one heard none of the query's changes, and unsubscribing it detached the first.
