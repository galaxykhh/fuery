...
- Speed up giving an observer a definition built again with the same key, as a widget's `build` does: the observer no longer hashes the key. `QueryObserver.setOptions` is 2.3 to 3.5 times faster, `QuerySlot.update` 1.9 to 3 times, `QueriesSlot.update` 1.7 to 2 times, and `MutationObserver.setOptions`, which hashed the key four times, 17 times. This holds for keys of strings, ints, bools, enums, lists, and maps with string keys, and other keys are still hashed.
- Speed up subscribing and unsubscribing an observer: both take the same time however many observers watch the query. With 10,000 on one query, they were 40 and 175 times slower.
- Fix: when an observer subclass overrides `==`, two equal observers of one query both hear its changes. Before, the second heard none of them, and unsubscribing it detached the first. A query now keeps its observers by identity, not by `==`.

## 1.5.0
- Add `MutationStateSlot`, which lists the state of every run of a mutation, oldest first, wherever the run was started. It is an `ObserverSlot` over the mutation cache. A `Mutation` finds its runs by its `mutationKey`, typed like the definition. `MutationFilters` find every matching run, typed `Object?`. Its `subscribe` listeners run in a microtask, once per batch, and only when the list changed. A definition without a key fails an assert. A run of other types under the key is left out and reported once to `onUncaughtError`.
- Add `MutationStateSlot.subscribeToRuns((previous, current) {...})` for side effects of any run. It reports each later change of each run, with that run's state before it. It never reports a state a run had when the listener was added, or a run the cache removed.
- Add the sealed `MutationStateSource`, which `Mutation` and `MutationFilters` implement.
- Add `QueryResult.observer` and `MutationResult.observer`, the observer that reported a result, so an adapter given only a result can listen to it. `InfiniteQueryResult.observer` is an `InfiniteQueryObserver`. A `QueryResult` built with its constructor has none.
- Add `ObserverSlot.listen((previous, current) {...})` on every slot, for side effects. It reports each later change in a microtask, with the result before it, never the result it starts from. When the slot moves to another observer, it starts over without a call and drops what the old one had queued. A listener that throws is reported to the client's `onUncaughtError`, or without it to the zone.
...
- Fix: a `QueriesSlot` `subscribe` listener that throws no longer stops the slot's other listeners. Its error goes to the client's `onUncaughtError`.
